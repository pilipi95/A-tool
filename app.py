...
main_nav = st.sidebar.radio(
    "",
    ["📊 Aktien-Analyse", "⭐ Watchlist", "🔍 Aktien-Screener"]
)
    ...
    if ticker_symbol:
        try:
            ...
            with tab_chart:
                ...
                with col_tf2:
                    # Dynamische Auswahl der Intervalle passend zum Zeitraum
                    if timeframe == "1d":
                ...
                # Daten mit gewähltem Intervall laden
    ...
    if st.session_state.watchlist:
        ...
        for w_sym in st.session_state.watchlist:
            try:
                ...
                # Kurse und Vortagesschluss ermitteln
                price_val = w_inf.get('currentPrice', w_inf.get('regularMarketPrice', 0))
                prev_close = w_inf.get('previousClose', price_val)
...
# ==========================================
    ...
    with col_sel1:
        ...
        # Neuer Filter für Tages-Signale
    ...
    if st.button("🚀 Screening starten", use_container_width=True):
        ...
        for idx, sym in enumerate(symbols_to_scan):
            ...
            try:
                ...
                # 2. Tages-Einstiegssignale erkennen
                today_signals = []
                ...
                # Bounce an 200-SMA (max 2% Abstand)
